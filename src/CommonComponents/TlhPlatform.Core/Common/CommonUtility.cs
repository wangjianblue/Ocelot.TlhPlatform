...
namespace TlhPlatform.Core.Common
{
    public class CommonUtility
    {
        ...
        public static void GetStreamLog(string path, Func<string, List<WebLogError>> func)
        {
            int bufferSize = 1024000;
            byte[] by = new byte[bufferSize];
            int tempCount = 0;
            FileStream fileStream = null;
            List<WebLogError> list = new List<WebLogError>();
            try
            {
                fileStream = new FileStream(path, FileMode.Open);
                long fileLength = fileStream.Length;
                int readCount = (int)Math.Ceiling((double)(fileLength / bufferSize));
                do
                {
                    fileStream.Read(by, tempCount * bufferSize, bufferSize);
                    tempCount++;
                    var LogMessage = Encoding.Default.GetString(by);
                    ...
                    if (LogMessage != null && LogMessage != "")
                    {
                        ...
                        var LogSplit = LogMessage.Split("==============================================================");
                        if (LogSplit != null && LogSplit.Length > 0)
                        {
                            for (int i = 0; i < LogSplit.Length; i++)
                            {
                                ...
                                if (!string.IsNullOrEmpty(item))
                                {
                                    ...
                                    {
                                        if (Lists[j].Contains("日志时间"))
                                            webLogError.LogLeve = Lists[j];
                                    }
        ...
        /// </summary>
        /// <param name="path"></param>
        public static IEnumerable<List<WebLogError>> GetStreamLog(string path)
        {
            FileStream fileStream = null;
            List<WebLogError> list = new List<WebLogError>();
            ...
            while (!streamReader.EndOfStream)
            {
                ...
                if (stream.Equals("=============================================================="))
                {
                    list.Add(JsonConvert.DeserializeObject<WebLogError>(sb.ToString()));
                    sb.Clear();
                    i++;
                    if (i % 2 == 0)
                    {
